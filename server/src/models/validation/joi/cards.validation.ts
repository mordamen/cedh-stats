const JoiCards = require('joi');

const createCardSchema = JoiCards.object({
	title: JoiCards.string().min(2).max(256).required().messages({
	...
	subTitle: JoiCards.string().min(2).max(256).required().messages({
	...
	description: JoiCards.string().min(2).max(1024).required().messages({
	...
	email: JoiCards.string().email({ minDomainSegments: 2 }).min(2).max(1024).required().messages({
	...
	image: JoiCards.object().keys({
		...
		alt: JoiCards.string().min(2).max(256).required().messages({
	...
	address: JoiCards.object().keys({
		state: JoiCards.string().min(0).max(256),
		country: JoiCards.string().min(2).max(256).required().messages({
		...
		city: JoiCards.string().min(2).max(256).required().messages({
		...
		street: JoiCards.string().min(2).max(256).required().messages({
		...
		houseNumber: JoiCards.number().min(1).required().messages({
		...
		zip: JoiCards.number().min(0).allow('').messages({
	...
	bizNumber: JoiCards.number().min(0).max(9999999).required().messages({
	...
	user_id: JoiCards.string().hex().length(24).messages({
...
const validateCardSchema = (userInput: any) => {
