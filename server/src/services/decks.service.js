 ...
 */
const deckStats = async (page = 1, query, colorID) => {
	const offset = (page - 1) * ITEMS_PER_PAGE;
	...
	const mergedResults = data.map((data) => {
		const matchingUniqueCards = uniqueCards.find((uniqueCards) => uniqueCards._id.includes(data.deckName));
		return {
			...data,
