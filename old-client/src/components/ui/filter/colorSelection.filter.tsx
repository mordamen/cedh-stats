...
const ColorSelection = () => {
	...
	return (
		<Popover>
			<PopoverTrigger>
				<Button className='border-dashed rounded-full text-xs m-1 p-2' variant='outline' size='sm'>
					...
					{selectedColors.length > 0 ? (
						<>
							...
							{selectedColors.map((colorLabel: string) => {
								const matchingIcon = colorIcons.find((icon) => icon.label === colorLabel);
								return (
									<>
										<Image
											key={colorLabel}
